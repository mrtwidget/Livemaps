            ...
            try
            {
                ...
                if (result == null)
                {
                    ...
                    MySQLCommand.CommandText =
                        ...
                        "enable_livemap TINYINT(1) DEFAULT 1," +
                        "enable_livemap_status TINYINT(1) DEFAULT 1," +
        ...
        public void UpdateServerData()
        {
            try
            {
                ...
                MySQLCommand.CommandText = string.Format(
                    result == null
                        ? "INSERT INTO {0} (server_id,server_name,app_version,map,online_players,max_players,is_pvp,is_gold,is_pro,has_cheats,hide_admins,cycle_time,cycle_length,full_moon,uptime,packets_received,packets_sent,port,mode,enable_livemap,enable_livemap_status,refresh_interval) VALUES (@server_id,@server_name,@app_version,@map,@online_players,@max_players,@is_pvp,@is_gold,@is_pro,@has_cheats,@hide_admins,@cycle_time,@cycle_length,@full_moon,@uptime,@packets_received,@packets_sent,@port,@mode,@enable_livemap,@enable_livemap_status,@refresh_interval)"
                        : "UPDATE {0} SET server_name=@server_name,app_version=@app_version,map=@map,online_players=@online_players,max_players=@max_players,is_pvp=@is_pvp,is_gold=@is_gold,is_pro=@is_pro,has_cheats=@has_cheats,hide_admins=@hide_admins,cycle_time=@cycle_time,cycle_length=@cycle_length,full_moon=@full_moon,uptime=@uptime,packets_received=@packets_received,packets_sent=@packets_sent,port=@port,mode=@mode,enable_livemap=@enable_livemap,enable_livemap_status=@enable_livemap_status,refresh_interval=@refresh_interval WHERE server_id=@server_id",
                ...
                MySQLCommand.Parameters.AddWithValue("@enable_livemap", (Livemap.Instance.Configuration.Instance.LivemapEnabled ? 1 : 0));
                MySQLCommand.Parameters.AddWithValue("@enable_livemap_status", (Livemap.Instance.Configuration.Instance.LivemapStatusEnabled ? 1 : 0));
